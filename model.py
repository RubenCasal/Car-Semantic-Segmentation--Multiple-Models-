...
class UNET(nn.Module):
    def __init__(
            self, in_channels=3, out_channels=1, features=[64, 128, 256, 512], dropout=0.0,
    ):
...
def test():
    x = torch.randn((1, 3, 480, 640))  # Batch size 1, 3 channels, 480x640 resolution
    model = UNET(in_channels=3, out_channels=1, dropout=0.3)  # Added dropout rate
    preds = model(x)
    print(f"Input shape: {x.shape}, Output shape: {preds.shape}")
    assert preds.shape == x.shape  # Ensure output shape matches input
